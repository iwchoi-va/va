...
public class SecretScriptResWebAction extends XwingWebAction {
	/**
	 * 
	 */
	...
	private String org2_flag = ""; //조직 2레벨
	...
	public void perform(JediRequest req, JediResponse res) throws WebActionException {
		ErrorLogger.info("SecretScriptResWebAction  Start!!");
		...
		cntr_cd = req.param.getString("cntr_cd", "");
		new_user = req.param.getBoolean("new_user", false);
		DS_DEPT = req.param.getListParam("DS_PARAM");
		ListParam DS_RES = null;
		
		
		DS_RES = ScriptBySclfcd();
		//else DS_RES = ScriptByQstcd();
		
		res.param.addValue("DS_RES",DS_RES);
		ErrorLogger.info("SecretScriptResWebAction END!!");

	}	
	
	
	public ListParam ScriptBySclfcd(){
		...
		for(int i =0; i< DS_DEPT.rowSize(); i++){
			...
			if("DEPT1".equals(type)){ 
				...
				if("N".equals(DS_DEPT.getValue(i, "ANL_USE_YN"))) continue;
		...
		try {
			...
			sqlParam.setSqlName("bis.bis020.secretScriptbyDept.sel");
			...
			DS_DEPT_RES = sqlResult.getListParam("bis.bis020.secretScriptbyDept.sel");
			...
			sqlParam1.setSqlName("bis.bis020.getsclfcd.sel");
			...
			ListParam DS_SCLF_CD = sqlResult1.getListParam("bis.bis020.getsclfcd.sel");
				...
				for(int i =0; i< DS_RES.rowSize();i++){			
					for(int j=0; j< obj.length;j++){						
						if(j < 2) obj[j] = DS_RES.getValue(i, j);
							...
							obj[j] = (Integer.parseInt(tot)-Integer.parseInt(cnt)) + "/" + tot;
						}
					}
	...
	public ListParam ScriptByQstcd(){
		ErrorLogger.info("########################ScriptByQstcd");
		...
		try {
			...
			IVRLogger.debug("ORG1 - " + org1_cd);
			sqlParam.setSqlName("bis.bis020.secretScriptQstcd.sel");
			sqlParam.addValue("sql", sql);
			sqlParam.addValue("new_user_id", new_user_sql);
			sqlParam.addValue("CED_NO", ced_no);
			sqlParam.addValue("SDATE", sdate);
			...
			sqlParam.addValue("ORG3_CD", org3_cd);
			sqlParam.addValue("USER_ID", user_id);
			sqlParam.addValue("CNTR_CD", cntr_cd);
			sqlParam.addValue("GRADE", grade);
			sqlParam.addValue("ANL_YN", anl_yn);
			...
			DS_DEPT_RES = sqlResult.getListParam("bis.bis020.secretScriptQstcd.sel");
