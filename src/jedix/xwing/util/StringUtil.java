...
public class StringUtil {
	...
	* @param    String      null check할 문자열 <BR>
	* @param    String      null인경우 return할 문자열 <BR>
	...
	*/
	...
	public static String getSocialid2Birthday(String socialid) {
		...
		if( socialid.length() >= 13 ) {
			...
			if( socialid.charAt(6) == '1' || socialid.charAt(6) == '2' ) result = "19" + result;
	...
	*
	*	@param str padding할 문자열
	*	@param width padding할 문자열의 총 길이
	*	@param width padding할 문자열의 총 길이
	...
	*/
	public static String getSocialId(String socialId){
		if(socialId==null)
			return "";
		if( socialId.length() < 6 ) return socialId;
		if(socialId.charAt(6)=='-') return socialId;
	...
	*
	*	@param socialId
	*	@param isFirst	 앞의 6자리 - true, 뒤쪽 7자리 - false
	*	@return 주민등록번호 앞6자리
	*/
	...
	/**
	*	문자열을 delimiter를 기준으로 분리하여 분리된 각 substring을 Vector로 리턴
	*   [ jdk1.4 java.lang.String.split(String deli)기능 ]
	*
	*	@param strString    - 처리대상 문자열
	*	@param strDelimeter - Delimiter
		...
		try {
			...
			result.add(0,srcString);
				...
				while ((srcString.indexOf(strDelimeter) > -1)) {
					...
					srcString = srcString.substring(lastIndex + strDelimeter.length(), srcString.length());
					cnt ++;
				}
