...
public class CapsPingCheckDb extends TimerTask {
	...
	public void run() {
		...
		//ErrorLogger.debug(Thread.currentThread().getName() + " ping check ����");
		//Logger.write("pingchekdb", Thread.currentThread().getName(),"ping check ����", "", "", "", "");

		SQLParam sqlParam = new SQLParam();

		try {
			sqlParam.clear();
			sqlParam.setSqlName("telecaps.sql.ping.db.list");
			SQLParam sqlResult = SQLServiceManager.getInstance().execute(sqlParam);

			//ErrorLogger.debug("telecaps.sql.ping.db.list sqlResult.getCount "
			//		+ sqlResult.getCount());
			...
			if (sqlResult.getCount() > 0) {
				...
				for (int i = 0; i < sqlResult.getCount(); i++) {
					a_device_id[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("DEVICE_ID");
					a_device_ip[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("DEVICE_IP");
					a_device_ip2[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("DEVICE_IP2");
					a_last_updated[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("LAST_UPDATED");
					a_cur_date[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("CUR_DATE");
					a_time_gap[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("TIME_GAP");
					a_alarm_grade[i] = sqlResult.getListParam("telecaps.res.ping.db.list").getParam(i).getString("ALARM_GRADE");

					//ErrorLogger.debug("Ping DB:" + a_device_id[i] + " "+ a_device_ip[i] + " " + a_last_updated[i] + " "+ a_cur_date[i] + " " + a_time_gap[i] + " "+ a_alarm_grade[i]);

					getAlarmInfo(a_device_ip[i], a_alarm_grade[i]);
					// ��� �˶��� ���
					if (!a_alarm_grade[i].equals("NO")) {
						// if(!JavaPingCheck(a_device_ip[i])) {
						if (!PingProcessCheck(a_device_ip[i])) {
							if (!a_device_ip2[i].isEmpty()) {
								if (!PingProcessCheck(a_device_ip2[i])) {
									//ErrorLogger.info("============ PingProcessCheck Next =============");
									//ErrorLogger.error("ping �˶�:"+ a_device_id[i] + " "+ a_device_ip[i] + " "+ a_last_updated[i] + " "+ a_cur_date[i] + " "+ a_time_gap[i] + " "+ a_alarm_grade[i]);
									//Logger.write("pingchekdb", Thread.currentThread().getName(),"ping �˶�: " + a_device_id[i] + " "+ a_last_updated[i] + " "+ a_cur_date[i] + " "+ a_time_gap[i], "", "","", "");
									insertAlarmMsg();
								} else {
									pingDateUpdate(a_device_id[i]);
						...
						/*
						 * } else { ErrorLogger.debug("22::" + a_device_id[i]);
						 * pingDateUpdate(a_device_id[i]);
						 * insertAlarmBackMsg(a_device_id[i]); }
						 */
						// ��� �˶� �ƴ� ���
	...
	public void pingDateUpdate(String device_id) {
		SQLParam sqlParam = new SQLParam();

		try {
			sqlParam.clear();
			sqlParam.setSqlName("telecaps.sql.ping.db.update");
	...
	public void getAlarmInfo(String device_ip, String alarm_grade) {
		...
		try {
			...
			sqlParam.clear();
			sqlParam.setSqlName("telecaps.sql.tcagnt.ping.db.gijun.select");
			sqlResult = SQLServiceManager.getInstance().execute(sqlParam);

			for (int i = 0; i < sqlResult.getCount(); i++) {
				alarm_msg_cd = sqlResult.getListParam("telecaps.res.tcagnt.ping.db.gijun.select").getParam(i).getString("ALARM_MSG_CD");
				alarm_msg = sqlResult.getListParam("telecaps.res.tcagnt.ping.db.gijun.select").getParam(i).getString("ALARM_MSG");
				interval = sqlResult.getListParam("telecaps.res.tcagnt.ping.db.gijun.select").getParam(i).getString("INTERVL");
				sms_yn = sqlResult.getListParam("telecaps.res.tcagnt.ping.db.gijun.select").getParam(i).getString("SMS_YN");
				s2o_yn = sqlResult.getListParam("telecaps.res.tcagnt.ping.db.gijun.select").getParam(i).getString("S2O_YN");
			}
	...
	public void insertAlarmMsg() {
		...
		//Logger.write("pingchekdb", Thread.currentThread().getName(), "�ߺ� �˶�: "
		//		+ same_msg_yn, "", "", "", "");
		//ErrorLogger.debug("[insertAlarmMsg] same_msg_yn:" + same_msg_yn);

		if ("N".equals(same_msg_yn)|| same_msg_yn.equals("")) {
			...
			try {
				...
				sqlParam.clear();
				//ErrorLogger.info("=============================== CapsPingCheckDb USE_YN : "+alarm_use_yn) ;
				sqlParam.setSqlName("telecaps.sql.tcagnt.alarm.insert");
				sqlParam.addValue("telecaps.sql.tcagnt.alarm.insert",
						lpSchValueAlarm);
				SQLServiceManager.getInstance().execute(sqlParam);
	...
	public void insertAlarmBackMsg(String t_device_id) {
		...
		try {
			sqlParam.clear();
			sqlParam.setSqlName("w_main.pingAlarm.message.cnt.check");
			sqlParam.addValue("device_id", t_device_id);
			sqlResult = SQLServiceManager.getInstance().execute(sqlParam);
			if (sqlResult.getCount() > 0) {
				if (sqlResult.getListParam("w_main.pingAlarm.message.cnt.check").getParam(0).getInt("ALARM_CNT") > 0) {
					sqlParam.clear();
					sqlParam.setSqlName("w_main.pingAlarm.message.update");
					sqlParam.addValue("device_id", t_device_id);
					sqlResult = SQLServiceManager.getInstance().execute(sqlParam);
					...
					sqlParam.clear();
					sqlParam.setSqlName("w_main.pingAlarm.clear.message.insert");
					sqlParam.addValue("w_main.pingAlarm.clear.message.insert",lpSchValueAlarm);
					SQLServiceManager.getInstance().execute(sqlParam);
				}
			}
	...
	public boolean JavaPingCheck(String t_device_ip) {
		try {
			InetAddress tagetIp = InetAddress.getByName(t_device_ip);
			boolean reachable = tagetIp.isReachable(3000);
	...
	public boolean PingProcessCheck(String t_device_ip) {
		InputStream is = null;
		BufferedReader br = null;
		try {
			Runtime run = Runtime.getRuntime();
			Process p = run.exec("C:\\windows\\system32\\ping.exe -n 10 "+ t_device_ip);
			is = p.getInputStream();
			...
			while ((line = br.readLine()) != null) {
				if (line.indexOf("Reply") >= 0 || line.indexOf("TTL") >= 0) {
					return true;
				} else if (line.indexOf("Request") >= 0|| line.indexOf("���") >= 0 || line.indexOf("����") >= 0) {
