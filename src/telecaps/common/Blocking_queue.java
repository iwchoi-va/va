 ...
 * from an empty queue. It's based on a linked list, so will never fill. (You'll
 * never block on a queue-full condition because there isn't one.)
 * 
 * <p>
 * This class uses the <code>LinkedList</code> class, introduced into the JDK at
 ...
 * <tr>
 * <td><font size=-1><i> <center>(c) 1999, Allen I. Holub.</center>
 * <p>
 ...
 * personal purposes, but you may not incorporate it into any commercial product
 * without express permission of Allen I. Holub in writing.</td>
 * </tr>
 * </table>
 * 
...
public class Blocking_queue {
	...
	/*******************************************************************
	 * The Closed exception is thrown if you try to used an explicitly closed
	 * queue. See {@link #close}.
	 */
	...
	/*******************************************************************
	 * Enqueue an item, and thereafter reject any requests to enqueue additional
	 * items. The queue is closed automatically when the final item is dequeued.
	 */
	 ...
	 * class is blocked, waiting to dequeue something. A timeout is not
	 * supported because of a potential race condition (see text). You can
	 * {@link Thread#interrupt interrupt} the dequeueing thread to break it out
	 * of a blocked dequeue operation, however.
	 * 
	 * @see #enqueue
	 * @see #drain
	 * @see #nonblocking_dequeue
	 * @return the dequeued object or null if the wait timed out and nothing was
	 *         dequeued.
	 */
	 ...
	 * </PRE>
	 * 
	 * To do the forgoing reliably, you must synchronize on the queue as
	 * follows:
	 * 
	 * <PRE>
	 * Blocking_queue queue = new Blocking_queue();
	...
	/*******************************************************************
	 * Return the number of threads waiting for a message on the current queue.
	 * See {@link is_empty} for warnings about synchronization.
	 */
	...
	/*******************************************************************
	 * Close the blocking queue. All threads that are blocked [waiting in
	 * dequeue() for items to be enqueued] are released. The {@link dequeue()}
	 * call will throw a {@link Blocking_queue.Closed} runtime exception instead
	 * of returning normally in this case. Once a queue is closed, any attempt
	 * to enqueue() an item will also result in a Blocking_queue.Closed
	 * exception toss.
	 * 
	 * The queue is emptied when it's closed, so if the only references to a
	 * given object are those stored on the queue, the object will become
	 * garbage collectable.
	 */
