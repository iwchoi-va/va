...
xwing.widget.Page._model = {
	...
	events : [ 'click', 'dblclick', 'mouseenter', 'mousemove', 'mouseleave', 'load', 'unload', 'resize' ]
};
...
xwing.widget.DataBindable._model = {
	attributes : {
		value : {
			type : 'string',
			defaultValue : null,
...
//								appendTo(jQuery(document.documentElement));
						...
						stop : function(event, ui){
							var node = jQuery(event.target.parentNode),
								...
								if(that.getRownum()) w -= that._rownumWidth;
							if (w < 0) w = 0;
...
xwing.widget.DataGridRow._model = {
		attributes : {
			height : {
				type : 'number',
				defaultValue : 22,
...
xwing.widget.DataGridColumn._model = {
		attributes : {
			width : {
				type : 'number',
				defaultValue : 100,
