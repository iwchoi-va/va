...
var _gubun = {D:["D","일별"],M:["M","월별"],H:["H","시간대별"]};
...
function g_createDate(obj, day_yn, mon_yn, hour_yn, label_v) { 
	
	var cnt = 0;
	if(day_yn=="Y") cnt++; if(mon_yn=="Y") cnt++; if(hour_yn=="Y") cnt++;
	if(label_v != undefined) label_txt = label_v;
...
//	}
	...
	var label = Xwing.createLabel({
		...
		valign : 'middle',
		halign : 'left',
		value : iif(label_txt!='',label_txt,'기간')
	});
	...
	var _date_gubun = [["P","과거(~6개월)"],["R","최근"]];
	var _ds_src_gb  = Xwing.createDataset("DS_SRC_GB");
	_ds_src_gb.setColumnInfo([ 'CODEID', 'CODENAME' ]);

	for(var i =0; i< _date_gubun.length;i++){
		_ds_src_gb.addRow([_date_gubun[i][0],_date_gubun[i][1]]);
	}
	...
	var _combo = Xwing.createCombo({
		...
		ns : "xwing",
		xw_type : "combo",
		value: _ds_src_gb.getValue(_ds_src_gb.size()-1,"CODEID")
	});
	_obj.appendChild(_combo);
	
	FUNC_XOBJECT.push(eval("$cmb_src_gb= Xwing.getCombo('cmb_src_gb')"));
	_left += eval("$cmb_src_gb.getWidth()")+_space;
...
//		value: _gubun[0][0]
	...
	//eval("$" + _gubunName + ".setAttribute('textpadding','4 5')");
	defalut_left = _left;
...
function makeWidget(){
	...
	//eval("$" + _sdateName + ".setAttribute('textpadding','4 5')");
	...
	//eval("$" + _edateName + ".setAttribute('textpadding','4 5')");
...
//	if(gubun_cnt > 1){
		...
		//eval("$" + _syearName + ".setAttribute('textpadding','4 5')");
		...
		//eval("$" + _smonthName + ".setAttribute('textpadding','4 5')");
		...
		//eval("$" + _eyearName + ".setAttribute('textpadding','4 5')");
		...
		//eval("$" + _emonthName + ".setAttribute('textpadding','4 5')");
//	}
...
//	if(gubun_cnt > 2){
		...
		//eval("$" + _dateName + ".setAttribute('textpadding','4 5')");
		...
		//eval("$" + _stimeName + ".setAttribute('textpadding','4 5')");
		...
		//eval("$" + _etimeName + ".setAttribute('textpadding','4 5')");
//	}
...
function betweenDate(startDt, endDt) {
    ...
    while (currentDate <= end) {
    	...
    	//currentDate.getMonth() 가 1씩 작게 나옴..
    	var m = ( currentDate.getMonth()< 9 ? "0"+(currentDate.getMonth()+1) : ""+(currentDate.getMonth()+1) ).toString();
...
function betweenDatehour(startDt, endDt) {
    var between = [];
    var currentDate = new Date(startDt.substring(0,4)+'-'+startDt.substring(4,6)+'-'+startDt.substring(6,8)), //new Date(parse(startDt)),
        end = new Date(endDt.substring(0,4)+'-'+endDt.substring(4,6)+'-'+endDt.substring(6,8)); //new Date(parse(endDt));
    while (currentDate <= end) {
...
function setDate(obj, gubun, sdate, edate){
	...
	/*var _param = {
			_sqlName : "cs.com.setDate_holiday.sel",
		sdate : sdate,
		edate : edate
	};
	...
	var opt = {
		sReqId : "_getHoliday",
		url : "service::cs.com.dateutil.do",
		param : _param,
		indicator : false
	};
	...
	Xwing.request(opt, function(sReqId, oRes, oErr, oXhr) {	
		var _ds_holiday=[];
		for(var i=0; i<oRes.DS_HOLIDAY.record.length; i++){
			_ds_holiday.push(oRes.DS_HOLIDAY.record[i].toString());
		}
		_callBack(obj, gubun, sdate, edate,_ds_holiday);
	});*/
...
function _callBack(obj, gubun, sdate, edate, _ds_holiday){
	...
	if(gubun == "D"){ 
		...
		for(var i=0; i<Xwing.getDataset(obj).size(); i++){ //요일붙이기
			...
			//alert("tmp:"+tmp+",yoilname:"+yoilname);
		...
		var tmpDate = betweenDatehour(sdate.substring(0,10), edate.substring(0,10));
...
//	}
	if(start_date>end_date){
		alert("조회 조건 날짜를 올바르게 선택해주세요.\n(조회시작날짜가 조회종료날짜보다 큽니다.)");
		return false;
	}
	return true;
}
...
/**
 * 검색 서버 분리로 인한 날짜 기준 체크하는 함수 추가
 * **/
function checkSrcFormat(){
	var gubun = Xwing.getCombo('cmb_gubun').getValue();
	var date = "";
	
	switch(gubun){
		case "H":
			date = Xwing.getDatepicker('date').getValue();			
			
			break;
		case "D":
			date = Xwing.getDatepicker('edate').getValue();
			
			break;
		case "M":
			date = Xwing.getSpin('eyear').getValue() + Xwing.getCombo('emonth').getValue();
			
			break;
	}
	
	if(!checkDateRange(date)) return false;
	
	return true;
}

function checkDateRange(date){
	var src_gubun = Xwing.getCombo('cmb_src_gb').getValue();
	var comp_date = g_addDate("m", -6, g_toDate(), "");
	
	if(date.length < 8) comp_date = comp_date.substr(0,6);
	
	if(src_gubun == "P"){
		if(date > comp_date){
			new xwing.Dialog.alert("시작일자가 6개월 이전 일자가 아닙니다.", "날짜조건 오류");
			return false;
		}
	}else if(src_gubun == "R"){
		if(date < comp_date){
			new xwing.Dialog.alert("종료일자가 6개월 이후 일자가 아닙니다.", "날짜조건 오류");
			return false;
		}
	}
