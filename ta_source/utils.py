...
class utils:
    ...
    def get_file_encoding_type(self, _filename):
        try:
            contents = open(_filename).read()
        except IOError as e:
            print("error", e)
            raise e
...
class log(object):
    ...
    def __init__(self, _loggerName, _logFileName):
        ...
        # 100MB = 1024*1024*100
        #filehandler = logging.handlers.RotatingFileHandler(_logFileName, mode='a', maxBytes=1024*1024*100, encoding="UTF-8") #backupCount=10,
        filehandler = logging.handlers.TimedRotatingFileHandler(_logFileName, when='midnight', backupCount=20, encoding="UTF-8")
    ...
    def __del__(self): #객체가 소멸하면 작동되는 메소드
        try:
            del self.logger
