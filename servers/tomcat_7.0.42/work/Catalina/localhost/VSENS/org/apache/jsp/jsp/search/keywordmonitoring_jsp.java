...
public final class keywordmonitoring_jsp extends org.apache.jasper.runtime.HttpJspBase
...
/**
*  subject: Keyword Monitoring ë©ì¸íì´ì§
*  ------------------------------------------------------------------------
...
*/
	...
	*   sort=ALL:RANK,1
	*/
	String sort			= getCheckReqXSS(request, "sort", "ALL@RANK/DESC,DATE/DESC");   //ì ë ¬íë
	...
	*/ 
	...
	String isMulti      = getCheckReqXSS(request, "isMulti", "n");        		
	
	String [] collections = COLLECTIONS;
	
	
	String yesterday = getCheckReqXSS(request, "yesterday", ""); 
	...
	String [] lastweek = getLastWeekCalendar("");
	String [] week = getWeekCalendar("20160107");
 	
	String nowDay = getNowDate();
	String nowHour = getNowHour();
	
	String lastMonth = getLastMonth();
	String addDate 	 = getAddOneDate();
	

//	daterange = daterange + "voc0@" + nowDay + "00" + "," + nowDay + nowHour + ";";  // ë¹ì¼ ìê°ë ì¹´ì´í¸ 
//	daterange = daterange + "voc1@" + nowDay + "00" + "," + nowDay + nowHour + ";";  // ë¹ì¼ ìê°ë í¤ìë ì¹´ì´í¸
//	daterange = daterange + "voc2@" + yesterday + "00" + "," + yesterday + nowHour + ";";  // ì ì¼ ëì¼ìê°ë ì¹´ì´í¸ 
//	daterange = daterange + "voc3@" + yesterday + "00" + "," + yesterday + nowHour + ";";  // ì ì¼ ëì¼ìê°ë í¤ìë ì¹´ì´í¸
	daterange = daterange + "voc0@" + nowDay + "," + nowDay + ";";  // ë¹ì¼ ìê°ë ì¹´ì´í¸ 
	daterange = daterange + "voc1@" + nowDay + "," + nowDay + ";";  // ë¹ì¼ ìê°ë í¤ìë ì¹´ì´í¸
	daterange = daterange + "voc2@" + yesterday  + "," + yesterday +  ";";  // ì ì¼ ëì¼ìê°ë ì¹´ì´í¸  
	daterange = daterange + "voc3@" + yesterday  + "," + yesterday +  ";";  // ì ì¼ ëì¼ìê°ë í¤ìë ì¹´ì´í¸

	
	daterange = daterange + "voc4@" + lastweek[0] + "," + lastweek[1] + ";";
	daterange = daterange + "voc5@" + lastweek[0] + "," + lastweek[1] + ";";
	daterange = daterange + "voc6@" + lastMonth + "01," + lastMonth + "31;";
	daterange = daterange + "voc7@" + lastMonth + "01," + lastMonth + "31;";

	//out.println("daterange=" + daterange);
	...
	collections = new String[collnum];
	String [][] coll_info = new String[collnum][24];
	String [] collectionsKor = new String[collnum];
	collectionsKor[0] = "Today_TotalCount";
	collectionsKor[1] = "Today_SearchCount";
	collectionsKor[2] = "Yesterday_TotalCount";
	collectionsKor[3] = "Yesterday_SearchCount";
	collectionsKor[4] = "Lastweek_TotalCount";
	collectionsKor[5] = "Lastweek_Searchcount";
	collectionsKor[6] = "Lastmonth_TotalCount";
	collectionsKor[7] = "Lastmonth_SearchCount";
	
	 
	for(int i=0; i<collnum; i++){
		collections[i] = "voc" + i; 
		String collsquery  = "<CONTENT:contains:"+query+">" ;
		if ( i % 2 == 0 ) collsquery = "";
		
		String [] temp =  {

			collections[i], // set index name
			"voc", // set collection name
			"0,3",  // set pageinfo (start,count)
			"1,1,1,1,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
			"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
			"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
			"DOCID,CONTENT,TERMS",// set search field
			"DOCID,CONTENT/100,SUMMARY,TERMS,TOPIC,NEG_LEVEL,Date,CATE_DATES,CATE_CD,CATE_NM,TACATE_CD,TACATE_NM,REASONCATE_CD,REASONCATE_NM,RULECATE_CD,RULECATE_NM,DURATION,EXTENSION,AGENT_ID,CONTACT_ID,REGIST_NO,INCALL_NO,OUTCALL_NO,OUT_GB,CUST_NAME,USER_ID,USER_NAME,DEPT_CD,DEPT_NAME,USER_WORK_MONTH,CUST_AGE,CUST_SEX,PROD_GRP,SVC_GRADE,MARK_GRADE,FILE_LEN",// set document field
			"", // set date range
			"", // set rank range
			"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
			collsquery, // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
			"", // set boost query (<fieldname:contains:value> | <field3:contains:value>...) and ' ', or '|'
			"", // set filter operation (<fieldname:operator:value>)
			"", // set groupby field(field, count)
			"", // set sort field group(field/order,field/order,...)
			...
			"", // set property group (fieldname,min,max, groupcount)
			"CATE_CD,CATE_NM,TACATE_CD,TACATE_NM,REASONCATE_CD,REASONCATE_NM,RULECATE_CD,RULECATE_NM,AGENT_ID,CONTACT_ID,REGIST_NO,INCALL_NO,OUTCALL_NO,OUT_GB,CUST_NAME,USER_ID,USER_NAME,SVC_GRADE,MARK_GRADE", // use check prefix query filed
			"NEG_LEVEL,Date,CUST_AGE,CUST_SEX,FILE_LEN", // set use check fast access field
			"", // set multigroupby field
			"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
			"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
			collectionsKor[i] // collection display name
	...
	wnsearch = new WNSearch(isDebug,isUIDSearch, collections, searchFields, coll_info);
		...
		if ( dateHash.size() > 0  ) {
			if ( dateHash.containsKey("ALL") ) {
				 ...
				 if ( strDate.indexOf("/") == -1 && strDate.indexOf(",") != -1 )  {
					 ...
					 arrDates[0] = arrDates[0].substring(0,4) + "/" + arrDates[0].substring(4,6) + "/" + arrDates[0].substring(6,8);
					 arrDates[1] = arrDates[1].substring(0,4) + "/" + arrDates[1].substring(4,6) + "/" + arrDates[1].substring(6,8);
...
//						 arrDates[0] = arrDates[0].substring(0,4) + "/" + arrDates[0].substring(4,6) + "/" + arrDates[0].substring(6,8);
//						 arrDates[1] = arrDates[1].substring(0,4) + "/" + arrDates[1].substring(4,6) + "/" + arrDates[1].substring(6,8);
					 } else if ( arrDates[0].length() == 10 ) {
						 arrDates[0] = parseDate(arrDates[0],"yyyyMMddHH","yyyy/MM/dd HH") + ":00:00";
						 arrDates[1] = parseDate(arrDates[1],"yyyyMMddHH","yyyy/MM/dd HH") + ":00:00";						 
					 }
					 wnsearch.setCollectionInfoValue(collections[i], DATE_RANGE, arrDates[0] + "," + arrDates[1] + ",-");
	...
	int returnCode = wnsearch.search("", isRealTimeKeyword, CONNECTION_CLOSE, useSuggestedQuery);
	...
	long endTime = System.currentTimeMillis();  //ì²ë¦¬ìê° ì¢ë£
