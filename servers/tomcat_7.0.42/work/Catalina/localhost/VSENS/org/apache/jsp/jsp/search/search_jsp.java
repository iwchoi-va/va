 ...
 * Generated at: 2016-05-12 05:05:53 UTC
...
*/
    ...
    **/
	...
	final static String PARAM_ARRAY_DELIMITER = "~";
	final static String SEARCH_ARRAY_DELIMITER = "~";
	final static String COMMON_ARRAY_DELIMITER = ":";
	


	/*
	* subject: 컬렉션 설정 파일
	* @original author: Moonsoo kim
	*/
	...
	public String[] COLLECTIONS = new String[]{"voc","excludeword","relation","hotkeyword","custanl"}; 
	...
	public String[][] COLLECTION_INFO = null;
		WNCollection(){
			COLLECTION_INFO = new String[][]
			{
				...
				"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
				"CONTENT",// set search field
				"DOCID,CONTENT/100,SUMMARY,TERMS,TERMS_COUNT,TERMS_KEYWORD,TERMS_SUMMARY,TOPIC,NEG_LEVEL,Date,CATE_DATES,CATE_CD,CATE_NM,TACATE_CD,TACATE_NM,REASONCATE_CD,REASONCATE_NM,RULECATE_CD,RULECATE_NM,DURATION,EXTENSION,AGENT_ID,CONTACT_ID,REGIST_NO,INCALL_NO,OUTCALL_NO,OUT_GB,CUST_NAME,USER_ID,USER_NAME,DEPT_CD,DEPT_NAME,USER_WORK_MONTH,CUST_AGE,CUST_SEX,PROD_GRP,PROD_NAME,PRODCATE_CD,PRODCATE_NM,SVC_GRADE,MARK_GRADE,FILE_LEN,RED_YN,ALIAS,ARS_CD,CHK_YN,CHANNEL",// set document field
				"", // set date range
				"", // set rank range
				...
				"CATE_CD,CATE_NM,TACATE_CD,TACATE_NM,REASONCATE_CD,REASONCATE_NM,RULECATE_CD,RULECATE_NM,AGENT_ID,CONTACT_ID,REGIST_NO,INCALL_NO,OUTCALL_NO,OUT_GB,CUST_NAME,USER_ID,USER_NAME,SVC_GRADE,MARK_GRADE", // use check prefix query filed
				"NEG_LEVEL,Date,DURATION,EXTENSION,REGIST_NO,CUST_AGE,CUST_SEX,FILE_LEN,RED_YN,ALIAS,ARS_CD,CHK_YN,CHANNEL", // set use check fast access field
				...
				"voc" // collection display name
				},
				{
				"excludeword", // set index name
				"excludeword", // set collection name
				"0,3",  // set pageinfo (start,count)
				"1,1,1,1,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
				"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
				"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
				"KEYWORD",// set search field
				"KEYWORD",// set document field
				"", // set date range
				"", // set rank range
				"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
				"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
				...
				"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
				"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
				"ExcludeWord" // collection display name
				},
				{
				"relation", // set index name
				"relation", // set collection name
				"0,3",  // set pageinfo (start,count)
				"1,1,1,1,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
				"RANK/DESC,DATE/DESC",  // set sort field (field,order) multi sort '/'
				"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
				"TERM",// set search field
				"TERM,RELATION",// set document field
				"", // set date range
				"", // set rank range
				"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
				"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
				...
				"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
				"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
				"Relation" // collection display name
				}
				,
				{
				"hotkeyword", // set index name
				"hotkeyword", // set collection name
				"0,3",  // set pageinfo (start,count)
				"1,1,1,1,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
				"RANKING/ASC,DATE/DESC",  // set sort field (field,order) multi sort '/'
				"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
				"KEYWORD",// set search field
				"KEYWORD,TYPE,RANKING,CHANGEMOUNT",// set document field
				"", // set date range
				"", // set rank range
				"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
				"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
				...
				"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
				"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
				"HotKeyword" // collection display name
				}
				,
				{
				"custanl", // set index name
				"custanl", // set collection name
				"0,3",  // set pageinfo (start,count)
				"1,1,1,1,0", // set query analyzer (useKMA,isCase,useOriginal,useSynonym, duplcated detection)
				"RANK/ASC,DATE/DESC",  // set sort field (field,order) multi sort '/'
				"basic,rpfmo,0",  // set sort field (field,order) multi sort '/'
				"DOCID",// set search field
				"ANL_GROUP_ID,ANL_KEY",// set document field
				"", // set date range
				"", // set rank range
				"", // set prefix query, example: <fieldname:contains:value1>|<fieldname:contains:value2>/1,  (fieldname:contains:value) and ' ', or '|', not '!' / operator (AND:1, OR:0)
				"", // set collection query (<fieldname:contains:value^weight | value^weight>/option...) and ' ', or '|'
				...
				"", // set auth query (Auth Target Field, Auth Collection, Auth Reference Field, Authority Query)
				"", // set Duplicate Detection Criterion Field, RANK/DESC,DATE/DESC
				"분석그룹" // collection display name
				}					
			};
		}
		
		WNCollection(String[] coll, String[][] coll_info) {
			COLLECTIONS = coll;
 ...
 */
    ...
    */
	...
	public static String getAddOneDate() {
        java.text.SimpleDateFormat dateFormat = new java.text.SimpleDateFormat
                ("yyyyMM", java.util.Locale.KOREA); 
		Calendar cal = Calendar.getInstance();
        ...
        try {
            ...
            for(int i=0; i<tnTokenCnt; i++) {
                if(toTokenizer.hasMoreTokens())	{
                	taRetVal[i] = toTokenizer.nextToken().trim();
                }
            }
        } catch (Exception e) {
	...
	public String xmlEscape(String originalUnprotectedString) {
...
//				break;
	...
	public String addQueryString(String str, String data, String addStr) {
	...
	/**
	* Request ì ë°°ì´ë¡ ë¤ì´ì¤ë ë´ì©ë¤ì í´ì¬íì´ë¸ì ë´ëë¤.
	* ì»¬ë ìë³ë¡ ì¡°ê±´ì ê±¸ê¸° ìí ê¸°ë¥ì¼ë¡ ê¸°ë³¸ êµ¬ë¶ìë ; ì´ë¤.
	* ì»¬ë ìêµ¬ë¶ì´ ìì¼ë©´ ì ì²´ ì»¬ë ìì ëí´ ì¡°ê±´ì´ ë¶ì¬ëë¤.
	* ex) collection1:value;collection2:value
	* collection ì ALL ì¼ ê²½ì°ìë ì ì²´ ì»¬ë ìì ëí ê¸°ì¤ì¼ë¡ íë¨íë¤.
	**/

	public Hashtable getRequestHash(String str) {
		return getRequestHash(str,";");
	}
	
	public Hashtable getRequestHash(String [] colls, String str) {
		return getRequestHash(colls, str,";");
	}	

	public Hashtable getRequestHashFilter(String str) {
		return getRequestHashFilter(str,";");
	}

	public Hashtable getRequestHash(String str,String split) {
		...
		if ( !str.equals("")) {
			...
			for ( int idx=0; idx < datas.length; idx++) {
				...
				if ( exs.length == 1 ) {
					...
					String merge = "";
					for ( int kdx=1;kdx<exs.length;kdx++) {
						merge = merge + exs[kdx];
						if ( kdx+1 < exs.length ) {
							merge = merge + ":";
						}
	...
	public Hashtable getRequestHash(String [] colls, String str,String split) {
		Hashtable hs = new Hashtable(); 
		if ( !str.equals("")) {
			String [] datas = str.split(split);
			for ( int idx=0; idx < datas.length; idx++) {
				...
				if ( exs.length == 2){
					if ( exs[0].equals("ALL") ) {
						for ( int kdx=0; kdx < colls.length; kdx++) {
							if ( hs.containsKey(colls[kdx]) ) {
								String hsdata = (String) hs.get(colls[kdx]);
								hsdata = hsdata + " " + exs[1];
								hs.remove(colls[kdx]);
								hs.put(colls[kdx],hsdata);
							} else {
								hs.put(colls[kdx],exs[1]);								
							}
						}
					} else {
	...
	public Hashtable getRequestHashFilter(String str,String split) {
		Hashtable hs = new Hashtable();
	...
	public String toJsonString(String key, Object value) {
		...
		StringBuffer sb = new StringBuffer();
		sb.append("\"" + key + "\":\"" + jsonEscape((String) value) + "\"");
		return sb.toString();
	...
	public String[] getLastWeekCalendar(String yyyymmdd) throws Exception{

		Calendar cal = Calendar.getInstance();
		int toYear = 0;
		int toMonth = 0;
		int toDay = 0;
		if(yyyymmdd == null || yyyymmdd.equals("")){   //íë¼ë©íê°ì´ ììê²½ì° ì¤ëë ì§
			toYear = cal.get(cal.YEAR);  
			toMonth = cal.get(cal.MONTH)+1;
			toDay = cal.get(cal.DAY_OF_MONTH);

			int yoil = cal.get(cal.DAY_OF_WEEK); //ìì¼ëì¤ê²íê¸°(ì«ìë¡)

			if(yoil != 1){   //í´ë¹ìì¼ì´ ì¼ìì¼ì´ ìëê²½ì°
			 yoil = yoil-2;
			}else{           //í´ë¹ìì¼ì´ ì¼ìì¼ì¸ê²½ì°
			 yoil = 7;
			}
			cal.set(toYear, toMonth-1, toDay-yoil);  //í´ë¹ì£¼ììì¼ë¡ ì¸í
		}else{
		...
		if(yoil != 1){   //í´ë¹ìì¼ì´ ì¼ìì¼ì´ ìëê²½ì°
			yoil = yoil-2;
			{
				cal.set(inYear, inMonth, inDay-yoil-7);  			
				String y = Integer.toString(cal.get(cal.YEAR));  
				String m = Integer.toString(cal.get(cal.MONTH)+1);
				String d = Integer.toString(cal.get(cal.DAY_OF_MONTH));
				if(m.length() == 1) m = "0" + m; 
			...
			{
				cal.set(cal.get(cal.YEAR), cal.get(cal.MONTH), cal.get(cal.DAY_OF_MONTH)+4);  
				String y = Integer.toString(cal.get(cal.YEAR));  
				String m = Integer.toString(cal.get(cal.MONTH)+1);
				String d = Integer.toString(cal.get(cal.DAY_OF_MONTH));
				if(m.length() == 1) m = "0" + m; 
			...
			yoil = 7;
			{
				cal.set(inYear, inMonth, inDay-yoil-6);  			
				String y = Integer.toString(cal.get(cal.YEAR));  
				String m = Integer.toString(cal.get(cal.MONTH)+1);
				String d = Integer.toString(cal.get(cal.DAY_OF_MONTH));
				if(m.length() == 1) m = "0" + m; 
	...
	public String[] getWeekCalendar(String yyyymmdd) throws Exception{
		...
		if(yoil != 1){   //í´ë¹ìì¼ì´ ì¼ìì¼ì´ ìëê²½ì°
			yoil = yoil-2;
			{
				cal.set(inYear, inMonth, inDay-yoil);  			
				String y = Integer.toString(cal.get(cal.YEAR));  
				String m = Integer.toString(cal.get(cal.MONTH)+1);
				String d = Integer.toString(cal.get(cal.DAY_OF_MONTH));
				if(m.length() == 1) m = "0" + m; 
...
public class WNCommon {
   ...
   public int setQueryLog(int val) {
       int ret =  search.w3SetQueryLog(val);
       return ret;
   }
   /**
...
//       ret = search.w3SetQueryLog(1);
   ...
   public int setBoostQuery(String collectionName, String query) {
       ...
       int option = Integer.parseInt(query.substring(idx+1, query.length()));
   ...
   */
   ...
   public int setCategoryBoost(String collectionName, String field,
       ...
       int ret = 0;
       ret = search.w3SetBoostCategory(collectionName, field, matchType, boostKeyword);
       return ret;
   }
   ...
   public int setBoostCategory(String collectionName, String field,
           String matchType, String boostKeyword) {
   ...
   public int addCategoryGroupBy(String collectionName, String[] categoryGroup) {        	
       ...
       for(int i=0; i < length; i++) {
			...
			if(dataFields.length == 2){
				//depthList depthì listë¥¼ ì½ë¡ (:) ì¼ë¡ êµ¬ë¶íì¬ ìë ¥
   ...
   public int addCategoryQuery(String collectionName, String[] categoryQuery) {
       ...
       for(int i=0; i < length; i++) { 
      	String[] dataFields = split(categoryQuery[i], COMMON_ARRAY_DELIMITER);
			if(dataFields.length == 2){
   ...
   */
   ...
   public void systemOut(String msg) {
	...
	public class WNSearch {
		...
		public WNSearch(boolean isDebug, boolean isUidSrch, String[] collections, String[] searchFields, String [][] coll_info){
		    this.common = new WNCommon();
		    this.wncol = new WNCollection(collections,coll_info);
		    this.collections = collections;
		    this.searchFields = searchFields;
		    this.sb = new StringBuffer();
		    this.warningMsg = new StringBuffer();
		...
		public int setBoostQuery(int idx) {
			...
			if (!wncol.COLLECTION_INFO[idx][BOOST_QUERY].equals("")) {
				//if(isDebug) {
				//	checkExFieldName("[w3SetBoostQuery] [setBoostQuery]", COLLECTIONS[idx],wncol.COLLECTION_INFO[idx][BOOST_QUERY] );
				//} 
...
//				ret = parseDate(common.getDate(collectionName, idx),"yyyy/MM/dd HH:mm:ss","yyyy.MM.dd");
		...
		*/
		...
		public int checkFilterFieldName(String msg, String collectionName, String value) {
			...
			for(int i=0; i < fieldName.length; i++) {
				...
				if(fieldName[i].indexOf("<", 0) != -1) {
					index = fieldName[i].indexOf("<", 0);
					int index2 = fieldName[i].indexOf(":");
					
					fieldName[i] = fieldName[i].trim().substring(index+1, index2);
					// 필터링 검색 필드 검사
		...
		public void debugMsg(boolean isRealQuery, String query){
			...
			for(int i=0; i < collections.length; i++) {
				...
				if(!isUidSrch) {
					...
					//키워드 부스팅  정보
					content = wncol.COLLECTION_INFO[idx][BOOST_QUERY];
					if(content != null && !content.equals("")){
						sb.append("<br>[w3SetCollectionQuery] " + content.replaceAll("<", "&lt;").replaceAll(">", "&gt;"));
					}					
					...
					//권한 Join 정보
					content = wncol.COLLECTION_INFO[idx][AUTH_QUERY];
					if(content != null && !content.equals("")){
						sb.append("<br>[w3SetAuthQuery] " + content);
					}
					//카테고리 그룹화  정보
					content = wncol.COLLECTION_INFO[idx][CATEGORY_GROUPBY];
					if(content != null && !content.equals("")){
						sb.append("<br>[w3AddCategoryGroupBy] " + content);
					}

					//카테고리 제약  정보
					content = wncol.COLLECTION_INFO[idx][CATEGORY_QUERY];
					if(content != null && !content.equals("")){
						sb.append("<br>[w3AddCategoryQuery] " + content);
					}
        ...
        public String getCollectionKorName(String col) {
            ...
            for ( int idx=0;idx< wncol.COLLECTION_INFO.length; idx++){
                String coll = wncol.COLLECTION_INFO[idx][COLLECTION_NAME].trim();
                if ( coll.equals(col) )
                    value =  wncol.COLLECTION_INFO[idx][COLLECTION_KOR];
				
            }
			if ( value == "" )  {
				for ( int idx=0;idx< wncol.COLLECTION_INFO.length; idx++){
					String coll = wncol.COLLECTION_INFO[idx][INDEX_NAME].trim();
					if ( coll.equals(col) )
						value =  wncol.COLLECTION_INFO[idx][COLLECTION_KOR];
	...
	public class MapList {
		...
		public MapList(String name, Object value) {
			super();
			this.name = name;
			this.value = value;
		}
	...
	public class SearchObject {
		...
		public SearchObject(String name, Object value) {
			list = new ArrayList();
			list.add( new MapList(name,value));
		}
		...
		public String toJSONString() {
			...
			for(int i=0;i<mapSize;i++) {
				MapList obj = (MapList) list.get(i);
				buf.append("\"");
				buf.append(obj.getName());
				buf.append("\":");

				if ( SearchArray.class.isInstance(obj.getValue())) {
					buf.append( ((SearchArray) obj.getValue()).toJSONString() );
				} else if ( SearchObject.class.isInstance(obj.getValue())) {
					buf.append( ((SearchObject) obj.getValue()).toJSONString() );
				} else if ( Integer.class.isInstance(obj.getValue())) {
					buf.append(  (Integer) obj.getValue() );
				} else {
					

					buf.append( "\"" );
					buf.append( jsonEscape( (String) obj.getValue() ) );
					buf.append( "\"" );
				}
		...
		public String toXMLString() {
			...
			for(int i=0;i<mapSize;i++) {
				...
				if ( SearchArray.class.isInstance(obj.getValue())) {
					buf.append("\n");
					buf.append( ((SearchArray) obj.getValue()).toXMLString() );
				} else if ( SearchObject.class.isInstance(obj.getValue())) {
					buf.append( ((SearchObject) obj.getValue()).toXMLString() );
				} else if ( Integer.class.isInstance(obj.getValue())) {
					buf.append( "<![CDATA[ " );
					buf.append( (Integer) obj.getValue() );
					buf.append( " ]]>" );
					...
					buf.append( "<![CDATA[ " );
					buf.append( (String) obj.getValue() );
					buf.append( " ]]>" );
				}
	...
	public class SearchArray {
		...
		public void add(SearchObject obj) {
			list.add(obj);
		}
		...
		public String toJSONString() {
			...
			if ( list != null && list.size() > 0 ) {
				...
				for(int i=0;i<listSize;i++) {
					SearchObject obj = (SearchObject) list.get(i);
  ...
  static {
    ...
    _jspx_dependants.put("/jsp/search/./common/./api/WNResult.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/./api/./WNAnchor.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/./api/./WNUtils.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/./api/.././api/WNDefine.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/./api/../WNCollection.jsp", Long.valueOf(1463026744012L));
    _jspx_dependants.put("/jsp/search/./common/./api/./WNCommon.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/SearchHeader.jsp", Long.valueOf(1459322041000L));
    _jspx_dependants.put("/jsp/search/./common/./api/WNSearch.jsp", Long.valueOf(1459322041000L));
  }
 ...
 request.setCharacterEncoding("UTF-8");
	...
	/*
	* subject: OpenAPI ìë ¥ ì¤ì  íì´ì§  
	* @original author: Moonsoo kim
	*/
	
	//html ë³´ìë¬¸ì  í´ê²°
	response.setHeader("Access-Control-Allow-Origin", "*"); 
    //ì¤ìê° ê²ìì´ íë©´ ì¶ë ¥ ì¬ë¶ ì²´í¬
    boolean isRealTimeKeyword = false;

	//ì¤í í ì¶ì² ê²ìì´ íë©´ ì¶ë ¥ ì¬ë¶ ì²´í¬
	...
	long startTime = System.currentTimeMillis(); //ì²ë¦¬ìê° ìì Start
	
	//ëë²ê¹ ë³´ê¸° ì¤ì 
	boolean isDebug = false; 
	  //uid search 
	boolean isUIDSearch = false; 
	
	String NEWLINE = "\n";
	
	String query		= getCheckReqXSS(request, "query", "");          //ê²ìì´
	String requery		= getCheckReqXSS(request, "requery", "");        //ê²°ê³¼ë´ ê²ìì´
	String rt			= getCheckReqXSS(request, "rt", "");             //ê²°ê³¼ë´ ì¬ê²ì ì²´í¬íë
	String rt2			= getCheckReqXSS(request, "rt2", "");            //ê²°ê³¼ë´ ì¬ê²ì ì²´í¬íë
	
	String type			= getCheckReqXSS(request, "type", "");            //ê²°ê³¼ë´ ì¬ê²ì ì²´í¬íë
	
	// ê²°ê³¼ ìì ëë²
	int pages				 = Integer.parseInt(getCheckReqXSS(request, "page", "1"));
	String totaldisplay		 = getCheckReqXSS(request, "totaldisplay", "3");
	String colldisplay		 = getCheckReqXSS(request, "colldisplay", "10");
	
	int startNo =  pages - 1  ; 
		
	String collection   = getCheckReqXSS(request, "collection", "ALL");  //ì»¬ë ìì´ë¦
		
	/*  ì ë ¬íë
	*	sort=sample_bbs@RANK/DESC;sample_kms:BBB/ASC
	*   sort=ALL:RANK,1
	*/
	String sort			= getCheckReqXSS(request, "sort", "");   //ì ë ¬íë
	
	/*  ê²ìíë ì ì 
	*   sfield=sample_bbs@TITLE,CONTENT;sample_kms:AAA,BBB
	*   sfield=ALL@TITLE,CONTENT
	*/
	String sfield		= getCheckReqXSS(request, "sfield", "");
	
	/*  ê²ìíë ì ì 
	*   rfield=sample_bbs@TITLE,CONTENT;sample_kms:AAA,BBB
	*   rfield=TITLE,CONTENT
	*/
	String rfield		= getCheckReqXSS(request, "rfield", "");
	
	/*  ExQuery ì¡°ê±´ íë ì ì  
	*   exquery=sample_bbs@[AUTH:contains:1111] [AUTH:contains:222];sample_kms@[AUTH:contains:1111] [AUTH:contains:222]
	*   exquery=ALL@[AUTH:contains:1111] [AUTH:contains:222]
	*/
	String exquery		= getCheckReq(request, "exquery", "");
	exquery = replace(exquery,"[","<");
	exquery = replace(exquery,"]",">");
	
	/*  collection query ì¡°ê±´ íë ì ì  
	*   collquery=sample_bbs@[FieldName:contains:1111] [FieldName:contains:222];sample_kms@[AUTH:contains:1111] [AUTH:contains:222]
	*/

	String collquery		= getCheckReq(request, "collquery", "");
	collquery = replace(collquery,"[","<");
	collquery = replace(collquery,"]",">");
 
	
	
	
	/*  FilterOperation ì¡°ê±´ íë ì ì  
	*   filter=sample_bbs@[TYPE:match:111] [SOURCE:match:111];sample_kms@[TYPE:match:111] [SOURCE:match:111]
	*   filter=ALL@[TYPE:match:111] [SOURCE:match:111]
	*/ 
	String filter		= getCheckReq(request, "filter", ""); 

	filter = replace(filter,"[","<");
	filter = replace(filter,"]",">");
	
	
	/*  Group By ì¡°ê±´ íë ì ì  
	*   groupby=sample@ALIAS:1/SC
	*/ 
	String groupby		= getCheckReq(request, "groupby", ""); 

	
	String catequery	= getCheckReq(request, "catequery", ""); 
	catequery = replace(catequery,"[","");
	catequery = replace(catequery,"]","");

	
	int topcount	= Integer.parseInt(getCheckReq(request, "topcount", "10000")); //topcount

	
	/*  ê¸°ê°ì¤ì  ì¡°ê±´ íë ì ì 
	*   daterange=sample_kms@1970/01/01,2030/01/01
	*   daterange=ALL@1970/01/01,2030/01/01
	*/ 
	
	String daterange	= getCheckReqXSS(request, "daterange", "");      //ììë ì§
	
	
	String isdebug		= getCheckReqXSS(request, "isDebug", "n");      //ëë²ê·¸ê° 
	String isAll		= getCheckReqXSS(request, "isAll", "n");
	String isAuth		= getCheckReqXSS(request, "isAuth", "y");
	String isUidSearch	= getCheckReqXSS(request, "isUid", "");            //uid search ì ë¬´
	String isEncode     = getCheckReqXSS(request, "isEncode", "y");        //filter operator, prefix íëì ëí´ ì¸ì½ë© ì ë¬´, ê¸°ë³¸ yes

	String isExclude	= getCheckReqXSS(request, "isExclude", "n");
	int mingroupcount	= Integer.parseInt(getCheckReq(request, "mingroupcount", "0")) ;
	
	String authReg	= getCheckReq(request, "authReg", ""); 
	String authUserId	= getCheckReq(request, "authUserId", ""); 
	String authContactId= getCheckReq(request, "authContactId", ""); 
	
	String authquery	= getCheckReq(request, "authquery", ""); 
	...
	Hashtable sortHash		= getRequestHash(sort); 
 
	//Hashtable totaldisplayHash = getRequestHash(totaldisplay);
	//Hashtable colldisplayHash = getRequestHash(colldisplay);

	Hashtable sfieldHash	= null;
	Hashtable rfieldHash	= null;		
	Hashtable exqueryHash	= null;		
	Hashtable collQueryHash	= null;		
	Hashtable filterHash	= null;		
	Hashtable catequeryHash	= null;		
	Hashtable groupbyHash	= null;		
	
	Hashtable dateHash		= null;
	
	if ( collections.length == 1 ) {
		sfieldHash		= getRequestHash(collections,sfield);
		rfieldHash		= getRequestHash(collections,rfield);
		exqueryHash		= getRequestHash(collections,exquery);	
		collQueryHash	= getRequestHash(collections,collquery);	
		filterHash		= getRequestHash(collections,filter);	
		catequeryHash	= getRequestHash(collections,catequery);	
		groupbyHash		= getRequestHash(collections,groupby,",");	
		dateHash		= getRequestHash(collections,daterange);	
	} else {
		sfieldHash		= getRequestHash(sfield);
		rfieldHash		= getRequestHash(rfield);
		exqueryHash		= getRequestHash(exquery);	
		collQueryHash	= getRequestHash(collquery);	
		filterHash		= getRequestHash(filter);	
		catequeryHash	= getRequestHash(catequery);	
		groupbyHash		= getRequestHash(groupby);	
		dateHash		= getRequestHash(daterange);	
	}
 
	
 
	WNSearch wnsearch = new WNSearch(isDebug,isUIDSearch, collections, searchFields);
   

	for (int i = 0; i < collections.length; i++) {  
        //ì¶ë ¥ê±´ì
		if ( collections.length > 1 ) {							
				int numStartNo = startNo * Integer.parseInt(totaldisplay);
				wnsearch.setCollectionInfoValue(collections[i], PAGE_INFO, numStartNo + "," + totaldisplay );
		} else {
				int numStartNo = startNo * Integer.parseInt(colldisplay);
				wnsearch.setCollectionInfoValue(collections[i], PAGE_INFO, numStartNo + "," + colldisplay );
		}
 
        //ê²ìì´ê° ìì¼ë©´ DATE_RANGE ë¡ ì ì²´ ë°ì´í° ì¶ë ¥
		if ( !query.equals("") ) {
			if ( sortHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], SORT_FIELD, (String) sortHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], SORT_FIELD, (String) sortHash.get(collections[i]) );
			}
		} else {
			if ( isAuth.equals("n") ) {
        ...
        //sfield ê°ì´ ìì¼ë©´ ì¤ì , ìì¼ë©´ ê¸°ë³¸ê²ìíë
		if ( sfieldHash.size() > 0 ) {
			if ( sfieldHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], SEARCH_FIELD, (String) sfieldHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], SEARCH_FIELD, (String) sfieldHash.get(collections[i]) );
			} 
		}
		
        //rfield ê°ì´ ìì¼ë©´ ì¤ì , ìì¼ë©´ ê¸°ë³¸ ê²°ê³¼íë 
		if ( rfieldHash.size() > 0 ) {
			if ( rfieldHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], RESULT_FIELD, (String) rfieldHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], RESULT_FIELD, (String) rfieldHash.get(collections[i]) );
			} 
		}
		
		//filter ì¤ì 
		if ( filterHash.size() > 0 ) {
			if ( filterHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], FILTER_OPERATION, (String) filterHash.get("ALL") );
			} else {
				if ( collections[i].equals("voc") ) {
					String imsiFilter = (String) filterHash.get(collections[i]);
					imsiFilter = imsiFilter + " <ALIAS:match:voc>";
					wnsearch.setCollectionInfoValue(collections[i], FILTER_OPERATION,  imsiFilter );
					
				} else {
					wnsearch.setCollectionInfoValue(collections[i], FILTER_OPERATION, (String) filterHash.get(collections[i]) );
        ...
        //exquery ì¤ì 
		if ( exqueryHash.size() > 0  ) {
			if ( exqueryHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], EXQUERY_FIELD, (String) exqueryHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], EXQUERY_FIELD, (String) exqueryHash.get(collections[i]) );
			}
		}
		
		//collquery ì¤ì 
		if ( collQueryHash.size() > 0  ) {
			if ( collQueryHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], COLLECTION_QUERY, (String) collQueryHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], COLLECTION_QUERY, (String) collQueryHash.get(collections[i]) );
			} 
		}
		
		
		//catequery ì¤ì 
		if ( catequeryHash.size() > 0  ) {
			if ( catequeryHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], CATEGORY_QUERY, (String) catequeryHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], CATEGORY_QUERY, (String) catequeryHash.get(collections[i]) );
			} 
		}
		...
		if ( collections[i].equals("voc")  ) {
			...
			if ( !authReg.equals("") ) {			
				wnsearch.setCollectionInfoValue(collections[i], AUTH_QUERY, "REGIST_NO,custanl,ANL_KEY,<ANL_GROUP_ID:contains:" + authReg + ">" );
			} else if ( !authUserId.equals("") ) {		
				wnsearch.setCollectionInfoValue(collections[i], AUTH_QUERY, "USER_ID,custanl,ANL_KEY,<ANL_GROUP_ID:contains:" + authUserId + ">" );
			} else if ( !authContactId.equals("") ) {		
				wnsearch.setCollectionInfoValue(collections[i], AUTH_QUERY, "CONTACT_ID,custanl,ANL_KEY,<ANL_GROUP_ID:contains:" + authContactId + ">" );
			} else if ( !authquery.equals("") ) {	
				wnsearch.setCollectionInfoValue(collections[i], AUTH_QUERY, "REGIST_NO,custanl,ANL_KEY,<ANL_GROUP_ID:contains:" + authquery + ">" );
			} 
			
		}		
								 
		if ( groupbyHash.size() > 0 ) {			
			if ( groupbyHash.containsKey("ALL") ) {
				wnsearch.setCollectionInfoValue(collections[i], CATEGORY_GROUPBY, (String) groupbyHash.get("ALL") );
			} else {
				wnsearch.setCollectionInfoValue(collections[i], CATEGORY_GROUPBY, (String) groupbyHash.get(collections[i]) );
			}  
		}

        //ê¸°ê° ì¤ì  , ë ì§ê° ììë
		if ( dateHash.size() > 0  ) {
			if ( dateHash.containsKey("ALL") ) {
				
				 String strDate = (String) dateHash.get("ALL");
				 if ( strDate.indexOf("/") == -1 && strDate.indexOf(",") != -1 )  {
					 String [] arrDates = strDate.split(",");
					 
					 if ( arrDates[0].length() == 8 ) {
						 arrDates[0] = parseDate(arrDates[0],"yyyyMMdd","yyyy/MM/dd");
					 } 

					 if ( arrDates[1].length() == 8 ) {
						 arrDates[1] = parseDate(arrDates[1],"yyyyMMdd","yyyy/MM/dd");
					 } 

					 
					 if ( arrDates[0].length() == 14 ) {
						 arrDates[0] = parseDate(arrDates[0],"yyyyMMddHHmmss","yyyy/MM/dd HH:mm:ss");
					 } 
					 
					 if ( arrDates[1].length() == 14 ) {
						 arrDates[1] = parseDate(arrDates[1],"yyyyMMddHHmmss","yyyy/MM/dd HH:mm:ss");
					 } 
 
					 wnsearch.setCollectionInfoValue(collections[i], DATE_RANGE, arrDates[0] + "," + arrDates[1] + ",-");
					 
				 } else {
 					 wnsearch.setCollectionInfoValue(collections[i], DATE_RANGE, (String) dateHash.get("ALL")  + ",-");
					 
				 } 
			} else {
				
				 String strDate = (String) dateHash.get(collections[i]);
				 if ( strDate.indexOf("/") == -1 && strDate.indexOf(",") != -1 )  {
					 String [] arrDates = strDate.split(",");
					 if ( arrDates[0].length() == 8 ) {
						 arrDates[0] = parseDate(arrDates[0],"yyyyMMdd","yyyy/MM/dd");
					 ...
					 wnsearch.setCollectionInfoValue(collections[i], DATE_RANGE, arrDates[0] + "," + arrDates[1] + ",-");
 					 ...
 					 wnsearch.setCollectionInfoValue(collections[i], DATE_RANGE, (String) dateHash.get(collections[i])  + ",-");
	...
	int returnCode = wnsearch.search(search, isRealTimeKeyword, CONNECTION_CLOSE, useSuggestedQuery);
...
/**
*  subject: OpenAPI 메인페이지
*  ------------------------------------------------------------------------
*  @original author: Moons
*  @update date 2016.03.12
*  ------------------------------------------------------------------------
*/
	SearchObject jsonMain = new SearchObject();

	// 디버그 메시지 출력
	String debugMsg = wnsearch.printDebug() != null ? wnsearch.printDebug().trim() : "";

	if ( isdebug.equals("y") ) {
		jsonMain.put("Debug",debugMsg.replaceAll("<br>","\r\n"));	
	} else {
		if ( !debugMsg.trim().equals("")) {
			jsonMain.put("Error",debugMsg.replaceAll("<br>","\r\n"));	
		}
	}

	// 전체건수 구하기
	if ( collection.equals("ALL") || isAll.equals("y") ) {
		for (int i = 0; i < collections.length; i++) {
	...
	if ( totalCount < -1 ) returnString = "Error";
	if ( returnCode != 0 ) {
			returnString = debugMsg.replaceAll("<br>","\r\n");	
	}
 
	jsonMain.put("Query",search);

	
	jsonMain.put("ReturnMsg",returnString);
	jsonMain.put("ReturnCode",returnCode);
	
	jsonMain.put("TotalCount",totalCount);

	SearchArray jsonArr1 = new SearchArray();
	
	if ( collection.equals("voc") )  {

		ArrayList<String> exWordList = new ArrayList<String>();
		if ( isExclude.equals("y") ) {
			String thisCollection = "excludeword";
			WNSearch exsearch = new WNSearch(isDebug,isUIDSearch, new String[]{thisCollection} , searchFields);
			exsearch.setCollectionInfoValue(thisCollection, PAGE_INFO, "0,1000" );
			exsearch.setCollectionInfoValue(thisCollection, DATE_RANGE, "1970/01/01,2035/01/01,-");
			exsearch.search("", isRealTimeKeyword, CONNECTION_CLOSE, useSuggestedQuery);
			
			String thisCollectionKor = exsearch.getCollectionKorName(thisCollection);
			int count = exsearch.getResultCount(thisCollection);
			int thisTotalCount = exsearch.getResultTotalCount(thisCollection);
 
			for(int idx = 0; idx < count; idx ++) {					 
				exWordList.add((String) exsearch.getField(thisCollection, "KEYWORD" , idx, false));
			}
			exsearch.closeServer();
		}		 
		 
		{		
		 
			String thisCollection = "voc";
			String thisCollectionKor = wnsearch.getCollectionKorName(thisCollection);
			int count = wnsearch.getResultCount(thisCollection);
			int thisTotalCount = wnsearch.getResultTotalCount(thisCollection);
			
			SearchObject jsonSub1 = new SearchObject();
			jsonSub1.put("CollName",thisCollection);
			jsonSub1.put("CollNameKor",thisCollectionKor);
			

			if ( groupbyHash.containsKey(thisCollection) ) {
				
				String DEPTH_DELIMITER = ":";
				
				String [] groups = ((String)groupbyHash.get(thisCollection)).split(";");
				SearchArray arrCategoryField = new SearchArray();
				SearchObject objCategoryField = new SearchObject();
				
				for ( int z=0; z < groups.length; z++) {
				
					String gValue = groups[z].split("/")[0];
					String gField = gValue.split(DEPTH_DELIMITER)[0];
					String [] gDepths = gValue.split(DEPTH_DELIMITER)[1].split(",");
					SearchArray arryGroup = new SearchArray();
					
									
					for(int gx=0; gx<gDepths.length; gx++){
						SearchObject Category = new SearchObject();
						int gDepth = Integer.parseInt(gDepths[gx]);
						int cateGroupCount = wnsearch.getCategoryCount(thisCollection,gField,gDepth); 
						SearchArray arrCateData = new SearchArray();		
						
						int num = 0;
						int exnum = 0; 
						for(int g=0; g<cateGroupCount; g++){
							
							String name = wnsearch.getCategoryName(thisCollection, gField, gDepth, g);
							int cateCount = wnsearch.getDocumentCountInCategory(thisCollection,  gField, gDepth, g);
							
							if ( name == null || name.equals("null")) continue;
							
							if ( cateCount < mingroupcount  ) continue;
							
							if ( exWordList.contains(name) )  {
								exnum ++;
								continue;
							} 
							
							
							if ( topcount  <=  num ) continue;		


							SearchObject cate1 = new SearchObject();	
							cate1.put("Name",name); 
							cate1.put("Count",new Integer(cateCount));
							arrCateData.add(cate1);
							num ++;
						}
	 
						Category.put("Depth",gDepth);				
						Category.put("CategoryCount",arrCateData.size());				
						Category.put("ExCludeWordCount",exnum);				
						Category.put("Category",arrCateData);				
						arryGroup.add(Category);

					}
					arrCategoryField.add(new SearchObject(gField,arryGroup));
					
				}
				
				jsonSub1.put("CategoryGroup",arrCategoryField);

			}
 
			if ( topcount == 10000 && mingroupcount == 0) {
					
				jsonSub1.put("TotalCount",thisTotalCount);
				jsonSub1.put("Count",count);

				if ( thisTotalCount > 0 ) {
		 
					SearchArray jsonArr2 = new SearchArray(); 
					
					for(int idx = 0; idx < count; idx ++) {
						
						SearchObject jsonSub2 = new SearchObject();
		 
						//jsonSub2.put("UID",  wnsearch.getField(thisCollection, "UID"  , idx, false));
						//jsonSub2.put("DATE", wnsearch.getField(thisCollection, "DATE" , idx, false));
						//jsonSub2.put("RANK", wnsearch.getField(thisCollection, "RANK" , idx, false));
						//jsonSub2.put("WEIGHT", wnsearch.getField(thisCollection, "WEIGHT" , idx, false));

						int colIdx = wnsearch.getCollIdx(thisCollection);
						String[] resultFields = split(wnsearch.wncol.COLLECTION_INFO[colIdx][RESULT_FIELD],","); 
						

						for(int kdx = 0; kdx < resultFields.length; kdx ++) {
							resultFields[kdx] = resultFields[kdx].split("/")[0];
 							boolean isCate4 = false;
							if ( thisCollection.equals("voc") ) {
								String strCate = wnsearch.getField(thisCollection,"CATE_CD",idx, false);
								String [] arrCate = strCate.split("#");
								for ( int x=0; x < arrCate.length; x++) {
									if ( arrCate[x].trim().startsWith("4^") ) {
										isCate4 = true;		
										break;
									}
								}
							...
							if ( resultFields[kdx].equals("SUMMARY") ) {
								String strs = wnsearch.getField(thisCollection,"SUMMARY",idx, false);;
								if ( isCate4 ) {
									StringBuffer sbSum = new StringBuffer();										
									String [] arrSummary = split(strs, ".");
									for ( int p=0; p < arrSummary.length; p++) {
										if (  arrSummary[p].replaceAll(" ","").length() > 10 ) { 
											if ( arrSummary[p].indexOf("NOT_MATCH") == -1 ) {
												sbSum.append(arrSummary[p] + "\\\\n");		
											}  
										}
									}
									jsonSub2.put(resultFields[kdx], sbSum.toString().replaceAll("NOT_MATCH",""));
									continue;
								}
								
								if ( strs.indexOf("NOT_MATCH") != -1 ) {
									StringBuffer sbSum = new StringBuffer();
									String [] arrSummary = split(strs, ".");
									...
									jsonSub2.put(resultFields[kdx], strs);			
								}
							}
									
							jsonSub2.put(resultFields[kdx], replace(wnsearch.getField(thisCollection, resultFields[kdx] , idx, false),"\n","\\\\n") );							
						} 
						jsonArr2.add(jsonSub2);
				   }
				   jsonSub1.put("Result",jsonArr2);
				}
			}
			
			
			jsonArr1.add(jsonSub1);
		} 
		jsonMain.put("Data",jsonArr1);
	...
	jsonMain.put("Time",((endTime - startTime) / 1000.0f ) + " (sec)");
	
	if ( type.equals("xml") ) {
		out.println(jsonMain.toXMLString());
	} else {
